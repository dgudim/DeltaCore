...
public class ParticleEffectPool {
    ...
    public ParticleEffectPool() {
        ...
        for (int i = 0; i < effectCollections.size; i++) {
            effectCollections.get(i).load();
        }
        log("prepared particle pool in " + TimeUtils.timeSinceMillis(time) + "ms", INFO);
    }
    
    public PooledParticleEffectCollection.PooledParticleEffect getParticleEffectByName(String name) {
        for (int i = 0; i < effectCollections.size; i++) {
            if (effectCollections.get(i).name.equals(name)) {
                return effectCollections.get(i).getParticleEffect();
            }
        }
        return null;
    }
    
    public PooledParticleEffectCollection.PooledParticleEffect getParticleEffectByPath(String path) {
        for (int i = 0; i < effectCollections.size; i++) {
